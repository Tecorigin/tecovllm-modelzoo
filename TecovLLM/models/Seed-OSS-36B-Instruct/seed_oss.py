...
from vllm_sdaa.weight_loader_utils import add_convert
...
class TecoSeedOssMLP(nn.Module):
    ...
    @property
    def weight_inter(self):
        return self.gate_up_proj.weight

    @property
    def weight_outer(self):
        return self.down_proj.weight

    def convert_weights(self):

        def convert_weight_inter(data):
            new_data = torch.empty_like(data, device="sdaa")
            torch.ops._C_lmk.weight_permute(new_data, None,
                                            data.permute(1, 0).contiguous(), 2,
                                            0, -1)
            return new_data

        def convert_weight_outer(data):
            new_data = torch.empty_like(data, device="sdaa")
            torch.ops._C_lmk.weight_permute(new_data, None,
                                            data.permute(1, 0).contiguous(), 3,
                                            0, -1)
            return new_data

        self.weight_inter.data = convert_weight_inter(self.weight_inter.data)
        self.weight_outer.data = convert_weight_outer(self.weight_outer.data)

    def forward(self, x):

        torch.ops._C_lmk.ffnv2(x, x, self.weight_inter, self.weight_outer,
                               None, "default", "swiglu")
...
class TecoRotaryEmbedding(RotaryEmbedding):
...
_ROPE_DICT: dict[tuple, TecoRotaryEmbedding] = {}
...
def get_rope_teco(
    ...
    if not rope_scaling:
        rotary_emb = TecoRotaryEmbedding(head_size, rotary_dim, max_position,
...
class SeedOssDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.input_layernorm = TecoRMSNorm(config.hidden_size,
                                           eps=config.rms_norm_eps)
        self.post_attention_layernorm = TecoRMSNorm(config.hidden_size,
                                                    eps=config.rms_norm_eps)
...
class SeedOssModel(nn.Module):
    ...
    def __init__(self,
        ...
        if get_pp_group().is_last_rank:
            self.norm = TecoRMSNorm(config.hidden_size,
                                    eps=config.rms_norm_eps)
        else:
...
class SeedOssForCausalLM(nn.Module, SupportsLoRA, SupportsPP):
    ...
    def load_weights(self, weights: Iterable[tuple[str,
        ...
        loaded_names = loader.load_weights(weights)
        for layer in self.model.layers:
            if hasattr(layer, "mlp") and isinstance(layer.mlp, TecoSeedOssMLP):
                layer.mlp.convert_weights()
